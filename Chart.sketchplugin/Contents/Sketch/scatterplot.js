...
/******/ 	return __webpack_require__(__webpack_require__.s = "./src/scatterplot.js");
/******/ })
/************************************************************************/
/******/ ({
...
/* harmony default export */ __webpack_exports__["default"] = (function (context) {
  ...
  function createScatterPlot() {
    ...
    if (params.scatterParams.bubbles === "true") {
      var maxDiametr = [],
          categories = dataObj.table.length / 3,
          maxDTemp = [];
      N = 3;

      for (var i = 0; i < dataObj.rows / 3; i++) {
        for (var j = 0; j < dataObj.columns; j++) {
          maxDTemp[j] = Number(dataObj.table[i * categories + 2][j]);
        }

        maxDiametr[i] = Math.max.apply(null, maxDTemp);
      }
    ...
    for (var _i = 0; _i < dataObj.rows / N; _i++) {
      for (var _j = 0; _j < dataObj.columns; _j++) {
        var maxValue = dataObj.niceMax,
            minValue = 0,
            _categories = dataObj.table.length / 2,
            diameter = params.scatterParams.diametr;

        if (params.scatterParams.bubbles === "true") {
          _categories = dataObj.table.length / 3;
        }
        ...
        scatterShape.setName("c_scatterPoint_" + (_i + 1) + "_" + (_j + 1));
        var scatterArray = NSArray.arrayWithArray([scatterShape]);

        if (canvas.doc.currentPage().currentArtboard() === null) {
          canvas.doc.currentPage().addLayers(scatterArray);
        } else {
          canvas.doc.currentPage().currentArtboard().addLayers(scatterArray);
        }

        scatterShape.select_byExpandingSelection(true, true);
      }
    }

    ;
    canvas.layer.select_byExpandingSelection(true, true);
    Object(_selectionToGroup__WEBPACK_IMPORTED_MODULE_1__["selectionToGroup"])(canvas, "Scatter plot");
  }
