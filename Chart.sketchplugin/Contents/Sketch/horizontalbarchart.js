...
/******/ 	return __webpack_require__(__webpack_require__.s = "./src/horizontalbarchart.js");
/******/ })
/************************************************************************/
/******/ ({
...
/* harmony default export */ __webpack_exports__["default"] = (function (context) {
  ...
  function createHorizontalBarChart() {
    ...
    dataObj.rows = dataObj.rows + 1; // Width of bar area

    var step = canvas.height / dataObj.columns; // Set margin between bars

    var margin = params.barWidthParam * step; // Set first point

    var y0 = canvas.y + margin,
        x0 = 0,
        x1 = 0,
        y1 = 0,
        maxValue = dataObj.niceMax,
        minValue = 0,
        zero = canvas.x; // Check negative values

    if (dataObj.niceMax <= 0) {
      maxValue = Math.abs(dataObj.niceMin);
      zero = canvas.x + canvas.width;
    ...
    for (var z = 0; z < dataObj.columns; z++) {
      mainArray[z] = zero + canvas.width / maxValue * (Number(dataObj.table[0][z]) + minValue);
    ...
    for (var _i = 0; _i < dataObj.rows - 1; _i++) {
      ...
      for (var j = 0; j < dataObj.columns; j++) {
        ...
        y1 = y0 + step - 2 * margin;
        x0 = mainArray[j];
        x1 = mainArray[j] + canvas.width / maxValue * Number(dataObj.table[_i + 1][j]);
        mainArray[j] = x1; // Create bar
        ...
        bar.closePath(); // Create barShape from path
        ...
        var barArray = NSArray.arrayWithArray([barShape]); // Add bar on artboard
    ...
    Object(_selectionToGroup__WEBPACK_IMPORTED_MODULE_1__["selectionToGroup"])(canvas, "Bar chart");
  }
