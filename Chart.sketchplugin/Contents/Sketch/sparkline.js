...
/* harmony default export */ __webpack_exports__["default"] = (function (context) {
  ...
  function createSparkline(Num) {
    // Set step by X between near points
    var xStep = canvas.width[Num] / (dataObj.columns - 1); // Set first point of line

    var x0 = canvas.x[Num],
        y0 = 0,
        maxValue = dataObj.niceMax,
        minValue = 0,
        zero = canvas.y[Num] + canvas.height[Num]; // Check negative values
    ...
    y0 = zero - canvas.height[Num] / maxValue * (Number(dataObj.table[Num][0]) + minValue); // Create line chart
    ...
    for (var j = 1; j < dataObj.columns; j++) {
      xNext = xLast + xStep;
      var y = zero - canvas.height[Num] / maxValue * (Number(dataObj.table[Num][j]) + minValue);
    ...
    border.color = MSColor.colorWithRed_green_blue_alpha(params.sparkParams.sparkColor[0] / 255, params.sparkParams.sparkColor[1] / 255, params.sparkParams.sparkColor[2] / 255, 1);
    border.thickness = params.sparkParams.borderThickness;
    lineShape.setName("line_" + (Num + 1));
    var lineArray = NSArray.arrayWithArray([lineShape]); // Add line and circle on artboard
    ...
    lineDot.frame = MSRect.rectWithRect(NSMakeRect(xLast - params.sparkParams.endWidth / 2, yLast - params.sparkParams.endWidth / 2, params.sparkParams.endWidth, params.sparkParams.endWidth));
    var lineDotShape = MSShapeGroup.shapeWithPath(lineDot);
    var fillDot = lineDotShape.style().addStylePartOfType(0);
    fillDot.color = MSColor.colorWithRed_green_blue_alpha(params.sparkParams.sparkColor[0] / 255, params.sparkParams.sparkColor[1] / 255, params.sparkParams.sparkColor[2] / 255, 1);
    lineDotShape.setName("dot_line_end");
    ...
    Object(_selectionToGroup__WEBPACK_IMPORTED_MODULE_1__["selectionToGroup"])(canvas, "Sparkline");
  }
