...
/******/ 	return __webpack_require__(__webpack_require__.s = "./src/gaugechart.js");
/******/ })
/************************************************************************/
/******/ ({
...
/* harmony default export */ __webpack_exports__["default"] = (function (context) {
  ...
  function createProgressChart(Num) {
    var radius = canvas.width[Num] / 2 - params.progressParams.progressBarBorder / 2,
        ...
        startAngle = -180,
        endAngle = dataObj.table[Num] * 1.8 - 180;
    ...
    arcShape.setName("a_gauge_" + 1);
    var arcArray = NSArray.arrayWithArray([arcShape]);
    var arcBack = NSBezierPath.bezierPath();
    arcBack.appendBezierPathWithArcWithCenter_radius_startAngle_endAngle(NSMakePoint(x0, yCenter), radius, startAngle, 0); // Create shape from path
    ...
    arcBackShape.setName("gaugeBack_" + 1);
    var arcBackArray = NSArray.arrayWithArray([arcBackShape]); // Add arc on artboard

    if (canvas.doc.currentPage().currentArtboard() === null) {
      canvas.doc.currentPage().addLayers(arcArray);
      canvas.doc.currentPage().addLayers(arcBackArray);
    } else {
      canvas.doc.currentPage().currentArtboard().addLayers(arcArray);
      canvas.doc.currentPage().currentArtboard().addLayers(arcBackArray);
    }

    arcShape.select_byExpandingSelection(true, true);
    arcBackShape.select_byExpandingSelection(true, true);
    canvas.layer[Num].select_byExpandingSelection(true, true);
    Object(_selectionToGroup__WEBPACK_IMPORTED_MODULE_1__["selectionToGroup"])(canvas, "Gauge chart");
  }
